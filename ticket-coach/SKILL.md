...
- If none of them ever escalated past pseudocode (including if no help was asked for at all), append one line: `{"date": "<today>", "axis": "<best-guess, e.g. autonomy or concreteness>", "signal": "completed <brief task description> with at most pseudocode-level help", "source_skill": "ticket-coach"}`. Phrase the signal as the positive-leaning observation it is, so `cognitive-profile` can weigh it against negative signals rather than reading it as neutral.
