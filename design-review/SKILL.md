...
Runs a holistic review pass over a consolidated design summary, catches issues that incremental approval missed, and either returns an approved design or escalates after the iteration budget is exhausted.

For the rules on dispatching the reviewer subagent (context isolation, type selection, model selection), see `../_shared/subagent-rules.md`. This skill is invoked by `brainstorming-committee` (step 7) and assumes no implementation begins until the design it returns has been approved by whoever is consuming it (the user, or `ticket-coach` when the committee is running inside a ticket-coach session).
...
2. **Dispatch the reviewer.** Use `task` to dispatch a reviewer subagent following `design-reviewer-prompt.md`. Paste the full design summary into the prompt. Do not pass session history. Use this model selection:

   | Stage      | Model order                                      | When to use                                                                                       |
   |------------|--------------------------------------------------|-----------------------------------------------------------------------------------------------------|
   | Default    | `haiku`                                          | First choice for every review.                                                                    |
   | Fallbacks  | `gpt-5.1-codex-mini`, then `gemini-2.5-flash-lite` | Use the first available model in this order. Use `gemini-2.5-flash-lite` only when Google models are explicitly enabled. |
   | Escalation | `sonnet`, then `gpt-5.2`, then `gemini-2.5-flash` | Use only when there is evidence that the design needs deeper reasoning, per `../_shared/subagent-rules.md`. |
