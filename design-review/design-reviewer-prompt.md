...
**Dispatch after:** Design summary has been consolidated (by `brainstorming-committee`, typically).

**Subagent type selection:** Replace `[REVIEWER_SUBAGENT_TYPE]` with `architect-reviewer` if available (it is built for evaluating system design decisions and patterns). Fall back to `general-purpose` only when `architect-reviewer` is not available. Per `../_shared/subagent-rules.md`, do not pre-escalate the model: pick one concrete default model, `haiku`, `gpt-5.1-codex-mini`, or `gemini-2.5-flash-lite`; escalate only on evidence to `sonnet`, `gpt-5.2`, or `gemini-2.5-flash`.

```
task ([REVIEWER_SUBAGENT_TYPE]):
